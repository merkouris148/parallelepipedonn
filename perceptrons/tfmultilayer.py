...
# Classes #
###########
class TFMLPBaseClass(
        abstractMLP.MLPAbstract,
        tf.keras.Sequential
    ):
    """
    Implementing the ``MLPBaseClass`` in TensorFlow.
    """
    ...
    def export2onnx(
        ...
        """
        A wrapper of `tf2onnx.convert.from_keras()`, see
        `here <https://github.com/onnx/tensorflow-onnx>`__.
        """
...
class FromArchitecture(TFMLPBaseClass):
    ## Constructor
    def __init__(
                    self,
        ...
        4. ``activation: bool``, a flag to either enable or disable the ``relu`` activations.
...
class FromWeights(TFMLPBaseClass):
    def __init__(
            self,
            Weights:        t.List[np.ndarray],
            biases:         t.List[np.ndarray],
        ...
        **Inputs:**
        ...
        5. ``activation: bool``, a flag to either enable or disable the ``relu`` activations.
